...
for i in range(7):
    ...
    if no_of_play_date == requested_day:
        courts_url = court_url_template.replace("<YYYY-MM-DD>", str(play_date))
        break
text = '\tChecking court availability for date: ' + str(req_day) + ' '\
       + str(play_date) + ' at ' + str(req_hour)
cprint(text, 'blue')
messages.append(play_date)
# because there are two tabs of courts on AM Tenis we need to go through both
cprint('\tLooking: ', 'green', end="")
for tab in range(2):
...
#    cprint('\tLooking: ', 'green', end="")
    for row in rows:
        ...
        for cell in cells:
            ...
            if column_no == 1:
                time_slot = cell.text
            # if any of cells in given time slot has value of RESERVE variable
